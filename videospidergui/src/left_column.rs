...
use iced::{Background, Border, Color, Shadow, border::Radius};
...
impl LeftColumn {
    ...
    pub fn view(&self) -> Element<StateChangedMessage> {
        let home_button = button("home")
            .width(Fill)
            .style(if self.state == State::Home {
        ...
        let container = container(column![home_button, search_button,].width(150).spacing(0))
...
fn button_style(_: &Theme, status: button::Status) -> button::Style {
    button::Style {
        background: match status {
            button::Status::Hovered => Some(Background::Color(Color::from_rgb8(75, 155, 255))),
            _ => Some(Background::Color(Color::from_rgb8(75, 75, 255))),
        },
